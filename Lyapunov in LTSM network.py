...
initials = [[np.pi/2, 1, np.pi/2, 1], [np.pi/4, -1, np.pi/4, -1], [-np.pi/2, 0, np.pi/2, 0]]
...
def f_pendulum(t, mean, sigma):
    θ1, θ2 = solve_pendulum(t, y0)
...
def solve_pendulum(t, y0):
    ...
    if system_type == 'simple' and angle_choice == 2:
        ...
        sol = solve_ivp(simple_pendulum, (t[0], t[-1]), y0[:2], t_eval=t, method='DOP853')
...
class LyapunovCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        for set_idx, (X_clean_seq, _, X_pert_seq) in enumerate(self.test_sets):
            lyaps = []
            lyaps_jac = []
            ...
            print(f"Epoch {epoch+1} | Test Set {set_idx+1}: ⟨λ⟩ = {round(np.mean(lyaps), 4)} s⁻¹, ⟨jac λ⟩ = {round(np.mean(lyaps_jac), 4)} s⁻¹")
...
def define_model():
    ...
    # z0 = layers.Input(shape=[None, 2])  # input is (window_size, 2)

    # z = layers.LSTM(128, return_sequences=False)(z0)

    # z = layers.Dense(64, activation='relu')(z)
    # z = layers.Dense(32, activation='relu')(z)
    # z = layers.Dense(16, activation='relu')(z)

    # z = layers.Dense(2)(z)  # Predict both θ and ω
...
def plot_lyapunov_comparison_all(lyap_lists, lyap_jac_lists, labels, train_loss, val_loss, ylabel=r'FTLE $\lambda$ [s⁻¹]'):
    ...
    for i in range(len(labels)):
        ax1.plot(lyap_lists[i], marker='o', label=f'{labels[i]} (pred)')
        ax1.plot(lyap_jac_lists[i], marker='x', linestyle='--', label=f'{labels[i]} (jac)')
    ...
    ax2 = ax1.twinx()
    ax2.plot(train_loss, color='black', linestyle='-', label='Train Loss', alpha=0.3)
    ax2.plot(val_loss, color='gray', linestyle='-', label='Val Loss', alpha=0.3)
    ax2.set_yscale('log')
...
def plot_lyapunov_noise(lyap_jac_clean, lyap_jac_noise, labels, ylabel=r'FTLE $\lambda$ [s⁻¹]'):
    ...
    plt.xlabel('Epoch')
    plt.ylabel(ylabel)
    plt.title('Lyapunov Exponents from Jacobian: Clean vs. Noisy Inputs')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def scatter_plot_predictions(model, X_test, y_test, title_suffix='Test Set'):
    ...
    # Theta
    plt.subplot(1, 2, 1)
    plt.scatter(θ_true, θ_pred, alpha=0.5)
    plt.plot([θ_true.min(), θ_true.max()], [θ_true.min(), θ_true.max()], 'r--')
    plt.xlabel('True θ [rad]')
    plt.ylabel('Predicted θ [rad]')
    plt.title(f'Angle Prediction ({title_suffix})')
    plt.grid(True)

    # Omega
    plt.subplot(1, 2, 2)
    plt.scatter(ω_true, ω_pred, alpha=0.5)
    plt.plot([ω_true.min(), ω_true.max()], [ω_true.min(), ω_true.max()], 'r--')
    plt.xlabel('True ω [rad/s]')
    plt.ylabel('Predicted ω [rad/s]')
    plt.title(f'Angular Velocity Prediction ({title_suffix})')
    plt.grid(True)
...
def plot_solution_curve():
    ...
    plt.suptitle(f'{system_type} Pendulum Solution')
...
# Parameters
system_type = 'double' # System: 'simple' or 'double'
...
# ================================== TRAINING ==================================
# Training data from one initial
X_train, y_train, X_pert_train, *_ = get_data_full_state(initials[0])  # Only one for training
...
#plot_lyapunov_comparison_all(lyap_cb.lyap_list_all,lyap_cb.lyap_list_jac_all,labels=labels)
plot_lyapunov_comparison_all(lyap_cb.lyap_list_all, lyap_cb.lyap_list_jac_all, labels=labels, train_loss=results.history['loss'], val_loss=results.history['val_loss'])

# plot_lyapunov_time(t, lp_network, 'Network FTLE')
